...
import { useAuth } from '@/contexts/AuthContext'; // Auth context provides user and role
import { Cake, CakeCategory, CakeSize, FrontendCreateCakeDto, FrontendUpdateCakeDto } from '@/types'; // Added DTOs
// Removed direct Firestore document/collection imports for CRUD, keep serverTimestamp for potential local use if needed (though API handles it)
// import { db } from '@/lib/firebase';
// import { doc, getDoc, setDoc, addDoc, collection, serverTimestamp } from 'firebase/firestore';
import { getCakeById, createCake as apiCreateCake, updateCake as apiUpdateCake } from '@/services/cakeApi'; // API functions
import LoadingSpinner from '@/components/ui/LoadingSpinner';
import { showErrorToast, showSuccessToast, showInfoToast } from '@/utils/toast';
import { getCakeImages, uploadCakeImages, deleteCakeImagesByUrls } from '@/services/storage'; // Storage functions remain
import { X } from 'lucide-react'; // Import X icon

const CakeForm: React.FC = () => {
  const { cakeId } = useParams<{ cakeId?: string }>();
  const navigate = useNavigate();
  const { user: bakerUser, role: bakerRole } = useAuth(); // Get user and role separately
  ...
  const initialCakeState: Partial<Cake> = {
    ...
    ingredients: [],
    allergens: [],
    sizes: [{ label: 'Standard', servings: 10, price: 0, priceModifier: undefined }], // Default size, ensure all fields
  };

  const [cakeData, setCakeData] = useState<Partial<Cake>>(initialCakeState); // Still use Partial<Cake> for form state
  ...
  useEffect(() => {
    if (isEditMode && cakeId) {
      ...
      const fetchCake = async () => {
        setLoading(true);
        try {
          const fetchedCake = await getCakeById(cakeId); // Use API
          if (fetchedCake) {
            // Use bakerRole from context for authorization check
            if (bakerUser?.uid !== fetchedCake.bakerId && bakerRole !== 'admin') { 
                showErrorToast("You are not authorized to edit this cake.");
                navigate("/baker-portal/manage-my-cakes"); // Or a general access denied page
                return;
            }
            setCakeData(fetchedCake); // API returns full Cake object
            // Existing image URLs are part of fetchedCake.images
            setExistingImageUrls(fetchedCake.images || []);
            setImagePreviews(fetchedCake.images || []);
          } else {
          ...
          showErrorToast(error.message || 'Failed to load cake details.');
          navigate('/baker-portal/manage-my-cakes');
        } finally {
          setLoading(false);
        }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Ensure all required fields for DTOs are present
      const dtoData: FrontendCreateCakeDto | FrontendUpdateCakeDto = {
        name: cakeData.name || '',
        category: cakeData.category || ('other' as CakeCategory),
        description: cakeData.description || '',
        price: Number(cakeData.price) || 0,
        images: finalImageUrls,
        isAvailable: cakeData.isAvailable ?? true,
        featured: cakeData.featured ?? false,
        ingredients: cakeData.ingredients || [],
        allergens: cakeData.allergens || [],
        sizes: cakeData.sizes && cakeData.sizes.length > 0 ? cakeData.sizes : [{ label: 'Standard', servings: 10, price: cakeData.price || 0 }],
        bakerId: cakeData.bakerId || bakerUser.uid, // Ensure bakerId is set, defaults to current user
      };
      
      // Remove undefined fields that are optional, to keep DTO clean
      Object.keys(dtoData).forEach(key => (dtoData as any)[key] === undefined && delete (dtoData as any)[key]);


      // 4. Save cake data via API
      if (isEditMode && cakeId) {
        await apiUpdateCake(cakeId, dtoData as FrontendUpdateCakeDto);
        showSuccessToast('Cake updated successfully!');
      } else {
        await apiCreateCake(dtoData as FrontendCreateCakeDto);
        showSuccessToast('Cake added successfully!');
      }
      navigate('/baker-portal/manage-my-cakes');
    } catch (error: any) {
      console.error("Error saving cake:", error);
      showErrorToast(error.message || `Failed to ${isEditMode ? 'update' : 'add'} cake.`);
    } finally {
      setLoading(false);
    }
  };
