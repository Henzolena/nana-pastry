              ...
              {order.items.map((item, index) => (
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between">
                      <h4 className="font-medium">{String(item.name || 'Unknown Cake')}</h4>
                      <p className="font-medium">{formatCurrency(item.price * Number(item.quantity || 0))}</p>
                    </div>
                    <p className="text-sm text-gray-500">Qty: {String(item.quantity || 0)}</p>
