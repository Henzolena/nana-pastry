...
import { getCakes } from '@/services/cakeApi'; // Import cakeApi function
  ...
  useEffect(() => {
    const fetchCakes = async () => {
      try {
        ...
        // Fetch all cakes using the API service
        const fetchedCakes = await getCakes(); // Use the cakeApi service
        
        // Filter for only available cakes for public display (can also be a backend filter)
        ...
        if (availableCakes.length > 0) {
          ...
          // Preserve 'All Cakes' and replace others
          const dynamicCategories = uniqueCategories.map(category => ({
            label: category.charAt(0).toUpperCase() + category.slice(1),
            value: category as CakeCategory,
          }));
          // Update categories state or the categories array directly if it's mutable and shared
          // For this component's 'categories' const, we need to be careful if it's meant to be static or reactive
          // Assuming 'categories' array is fine to be mutated here for dynamic options:
          categories.length = 1; // Keep 'All Cakes'
          categories.push(...dynamicCategories);
        }

      } catch (err: any) { // Catch any error type
        console.error("Error fetching cakes:", err);
        const errorMessage = err.message || "Failed to load cakes. Please try again later.";
                ...
                {filteredCakes.map((cake) => (
                  ...
                  >
                    <div className="relative overflow-hidden rounded-t-xl">
                      <img 
                        src={(cake.images && cake.images.length > 0 ? cake.images[0] : cake.imageUrl) || '/placeholder-cake.jpg'}
                        alt={cake.name}
