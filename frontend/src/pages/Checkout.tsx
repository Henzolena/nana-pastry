...
import { createNewOrder, OrderItem } from '@/services/order';
import { useAuth } from '@/contexts/AuthContext';

import CustomerInfoForm, { CustomerInfo } from '@/components/checkout/CustomerInfoForm';
import DeliveryOptionsForm, { DeliveryInfo as DeliveryInfoType, DeliveryMethod } from '@/components/checkout/DeliveryOptionsForm'; // Import DeliveryMethod
...
const Checkout: React.FC = () => {
  ...
  const handlePaymentSubmit = async (paymentInfo: PaymentInfo) => { // Made async
    ...
    const orderData: {
      ...
      deliveryMethod: DeliveryMethod; // Use the imported DeliveryMethod type
      customerInfo: {
        name: string;
        email: string;
        phone: string;
      ...
      pickupInfo?: { // Matches OrderData: pickupInfo is optional
        pickupDate: string;    // Matches OrderData: required if pickupInfo exists
        pickupTime: string;    // Matches OrderData: required if pickupInfo exists
        storeLocation?: string; // Matches OrderData: optional
      };
    } = {
      items: orderItems,
    ...
    if (checkoutState.deliveryInfo?.method === 'delivery') {
      ...
      // Add delivery date if available and format as string
      if (checkoutState.deliveryInfo.deliveryDate) {
        try {
          ...
          orderData.deliveryInfo.deliveryDate = date.toISOString();
        } catch (error) {
    ...
    if (checkoutState.deliveryInfo?.method === 'pickup') {
      orderData.pickupInfo = {
        ...
        // Default to current date formatted as string
        pickupDate: new Date().toISOString() 
      };
      
      // Add pickup date if available and format as string
      if (checkoutState.deliveryInfo.pickupDate) {
        try {
          const date = new Date(checkoutState.deliveryInfo.pickupDate);
          // Format date as ISO string for backend
          orderData.pickupInfo.pickupDate = date.toISOString();
        } catch (error) {
    ...
    try {
      ...
      clearCart();
      // Updated success toast to include email confirmation notice
      showSuccessToast(`Order placed successfully! A confirmation email has been sent to ${orderData.customerInfo.email}.`);
