...
import { collection, query, where, getDocs, deleteDoc, doc, addDoc, serverTimestamp } from 'firebase/firestore'; // Import Firestore functions
import LoadingSpinner from '@/components/ui/LoadingSpinner';
import { showErrorToast, showSuccessToast, showInfoToast } from '@/utils/toast';
import { cakes as staticCakes } from '@/utils/data'; // Import static cake data
// We'll also need a function to delete images from storage if cakes have images
// import { deleteCakeImage } from '@/services/storage'; // Placeholder
...
const MyCakesList: React.FC = () => {
  ...
  const handleImportStaticCakes = async () => {
    ...
    try {
      ...
      for (const staticCake of staticCakes) {
        ...
        const cakeToImport: Partial<Cake> & { createdAt?: any, updatedAt?: any } = { // Add timestamps to type
  ...
  const fetchBakerCakes = async () => {
    ...
    try {
      const cakesCollectionRef = collection(db, 'cakes');
      const q = query(cakesCollectionRef, where('bakerId', '==', bakerUser.uid));
      const querySnapshot = await getDocs(q);
      const bakerCakes = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Cake));
      setCakes(bakerCakes);
    } catch (error) {
      console.error("Error fetching baker's cakes:", error);
      showErrorToast("Failed to fetch your cakes.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeleteCake = async (cakeId: string, imageUrl?: string) => {
    ...
    try {
      ...
      // if (imageUrl) {
      //   try {
      //     await deleteCakeImage(imageUrl); // This function needs to be created in storage.ts
      //     showInfoToast('Cake image deleted from storage.');
      //   } catch (imgError) {
      //     console.error("Error deleting cake image:", imgError);
      //     showErrorToast('Cake data deleted, but failed to delete image from storage.');
      //   }
      // }
      ...
      showErrorToast('Failed to delete cake.');
      setLoading(false);
    }
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredCakes.map((cake) => (
            <div key={cake.id} className="border border-warmgray-200 rounded-lg p-4 shadow-sm hover:shadow-lg transition-shadow duration-200 flex flex-col">
              <img src={cake.imageUrl || '/public/images/cakes/default-cake.png'} alt={cake.name} className="w-full h-48 object-cover rounded-md mb-3" />
              ...
              <div className="mt-auto flex space-x-2 pt-3 border-t border-warmgray-100">
                ...
                <button
                  onClick={() => handleDeleteCake(cake.id, cake.imageUrl)} // Use Firestore document ID
