      ...
      <div className="p-6">
        ...
        <div className="mb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <p>{formatDate(order.createdAt.toDate(), { type: 'dateTime' })}</p>
          </div>
          <div>
              ...
              <span className="text-xs text-gray-500 ml-1">
                {order.statusHistory && order.statusHistory.length > 0 
                  ? `Last updated: ${formatDate(
                      order.statusHistory[order.statusHistory.length - 1].timestamp instanceof Date 
                        ? order.statusHistory[order.statusHistory.length - 1].timestamp
                        : typeof order.statusHistory[order.statusHistory.length - 1].timestamp === 'object' && 
                          'toDate' in order.statusHistory[order.statusHistory.length - 1].timestamp 
                            ? (order.statusHistory[order.statusHistory.length - 1].timestamp as any).toDate()
                            : order.statusHistory[order.statusHistory.length - 1].timestamp, 
                      { type: 'dateTime' }
                    )}`
                  : 'No status updates'
                    ...
                    <tr key={payment.id || index}>
                      <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">
                        {formatDate(
                          ...
                          typeof payment.date === 'object' && 'toDate' in payment.date 
                            ? payment.date.toDate() 
                            : payment.date, 
                          { type: 'dateTime' }
                        )}
